...
- For `refund` (`CARD` and wallets): validates the original `payment` is `SETTLED`, belongs to the vendor, is within its refund eligibility window, and that this `refund`'s amount plus its `refundedAmount` so far does not exceed the original amount (partial and multiple `refund`s allowed, unlike `void`). Owns and increments `refundedAmount` on the original `payment` itself, analogous to how `threeds-service` owns the `authenticate` ceiling.
