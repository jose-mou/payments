...
- For `refund` (`CARD` and wallets): validates the original `payment` is `SETTLED`, belongs to the vendor, and tracks the **cumulative refunded amount** against it — this `refund`'s amount plus all prior successful `refund`s referencing the same `payment` must not exceed its original amount (partial and multiple `refund`s allowed, unlike `void`). Owns this running total itself, analogous to how `threeds-service` owns the `authenticate` ceiling.
...
**Refund variant (`refund`)**: reverses an already-`SETTLED` `payment`, **partially or fully, possibly across multiple `refund`s**. Runs through the same pipeline as any other operation (steps 3–6): `card-transactions-service` validates the original `payment` is `SETTLED`, belongs to the vendor, and that this `refund`'s amount plus the cumulative amount of prior successful `refund`s against it does not exceed the `payment`'s original amount; `tid-assignment-service` allocates a TID; `<acquirer-bank>-tx-service` sends the reversal to the bank — a genuinely new, separately-settling money movement, unlike `void` (which prevents money from moving in the first place, is full-amount only, and at most once per transaction — no cumulative tracking needed there). On success, the original `payment` transitions to `REFUNDED` once fully refunded; otherwise it remains `SETTLED` (see open question in the application requirements spec on whether a distinct partially-refunded state is needed).

> **Open concern (2026-07-13):** the active-database purge policy (see Data Lifecycle & Scalability) removes a `SETTLED` transaction once its operational window has expired and its presence in the historical store is verified. `refund` needs to validate against the *original* transaction (FR-O9) **and** track its cumulative refunded amount so far (now that partial/multiple `refund`s are allowed) — but real-world refunds can arrive well after that window, potentially after the row is purged. Whether `refund` validation and the running cumulative-refunded total live in the active database (requiring `SETTLED` transactions to stay un-purged for as long as they remain refundable) or in the historical store (a new cross-store read/write path not otherwise used by processing) is unresolved — needs a decision before `refund` is implemented.
