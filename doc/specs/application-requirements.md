...
5. **Refund eligibility window trade-off (FR-O8/NFR-7):** the 180-day default is confirmed and now configurable in `card-transactions-service` (see `Resolved`), but the underlying trade-off remains open: the longer this window, the longer `SETTLED` `payment`s must stay in the active database, working against the whole point of the active/historical split (keeping the hot-path database lean, `ARCHITECTURE.md` §Data Lifecycle & Scalability). If the intent is to eventually match the ~13-month scheme dispute window rather than a shorter merchant refund policy, that cost should be reconsidered explicitly.
6. **Cross-service config consistency (NFR-7):** `card-transactions-service`'s refund eligibility window and `gateway-service`'s active-database purge threshold are two independent configuration values (database-per-service, no shared config) that must be kept consistent operationally — nothing in the platform enforces this automatically. Worth deciding whether this warrants a stronger safeguard (e.g. a deployment-time check, or deriving the purge threshold from the refund window rather than configuring both independently) before this ships.
