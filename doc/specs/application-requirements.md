...
- **FR-S1** — Settlement: per acquirer, all money-moving operations (`AUTHORISED` `payment`s not `VOIDED`, and successful `refund`s) of a cutoff window are written to that bank's settlement file and delivered over SFTP at the bank's cutoff time/timezone. Inclusion in a confirmed batch transitions a `payment` to `SETTLED`; a `refund` settles in the opposite direction, incrementing the original `payment`'s `refundedAmount` (FR-D1) and, once it reaches the `payment`'s full amount, transitioning it to `REFUNDED` — otherwise it remains `SETTLED` (partially refunded; further `refund`s still possible — confirmed 2026-07-17: no distinct partially-refunded state; `refundedAmount` in the snapshot is how vendors/reporting distinguish the case).
