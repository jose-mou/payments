...
- **FR-O8 — `refund`**: reverses a `payment` that has **already been settled** — the post-settlement counterpart of `void` (FR-O6). Unlike `void`, **partial and multiple `refund`s are allowed** against the same `payment`: one or more `refund` transactions may reference it, as long as `refundedAmount` (FR-D1) plus this `refund`'s amount does not exceed the original `payment`'s amount. Only successful (`AUTHORIZED`) `refund` attempts increment `refundedAmount`; declined/failed/rejected attempts do not. A `refund` is only accepted within a **refund eligibility window** from settlement (default 180 days — an assumption, no value was given; see open questions on the tension this creates with the active-database purge policy). A `refund` is itself a new acquirer-facing money movement: it runs through the same pipeline as any other operation (validation → TID → acquirer, FR-O7) since funds must actually be returned rather than simply withheld, and settles in its own right, in the opposite direction (`settlement-service` models this — no file-format change needed). Once `refundedAmount` reaches the original `payment`'s full amount, it transitions `SETTLED` → `REFUNDED` (§5); partially refunded, it remains `SETTLED` (further `refund`s still possible up to the remaining amount).
...
- **FR-S1** — Settlement: per acquirer, all money-moving operations (`CAPTURED` `payment`s not `VOIDED`, and successful `refund`s) of a cutoff window are written to that bank's settlement file and delivered over SFTP at the bank's cutoff time/timezone. Inclusion in a confirmed batch transitions a `payment` to `SETTLED`; a `refund` settles in the opposite direction, incrementing the original `payment`'s `refundedAmount` (FR-D1) and, once it reaches the `payment`'s full amount, transitioning it to `REFUNDED` — otherwise it remains `SETTLED` (partially refunded; further `refund`s still possible; see open question below on whether a distinct partially-refunded state is needed instead).
...
- **NFR-7 — Retention:** historical store ≥ 13 months (scheme dispute window); active database holds in-flight transactions, those within their operational window, **and any `SETTLED` `payment` still within its refund eligibility window** (FR-O8, default 180 days) — a `refund` must be able to validate against, and update `refundedAmount` on, the original transaction, so it cannot be purged from the active database before that window closes (see `ARCHITECTURE.md` §Data Lifecycle & Scalability); idempotency keys ≥ 24 h.
...
5. **Refund eligibility window (FR-O8/NFR-7):** default 180 days is an assumption, no value was given — confirm or adjust. Also flags a real trade-off: the longer this window, the longer `SETTLED` `payment`s must stay in the active database (NFR-7), working against the whole point of the active/historical split (keeping the hot-path database lean, `ARCHITECTURE.md` §Data Lifecycle & Scalability). If the intent is to match the ~13-month scheme dispute window rather than a shorter merchant refund policy, that cost should be reconsidered explicitly rather than inherited by default.
